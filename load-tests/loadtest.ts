  ...
  return new Promise((resolve, reject) => {
    ...
    socket.on("connect", () => {
      ...
      if (isHost) {
        ...
        socket.emit("join_room", { roomCode, nickname }, (res: any) => {
          if (!res.success)
...
async function run() {
  try {
    ...
    sockets[0].emit("start_game", { roomCode }, (res: any) => {
    ...
    sockets[0].on("game_start", (data: any) => {
    ...
    sockets[0].on("countdown_start", (data: any) => {
    ...
    sockets[0].on("game_ended", (data: any) => {
      gameActive = false;
      console.log(
