...
import type { RoomState, PlayerState, GameEndReason } from '@typeordie/shared'
...
export function useGameEvents(socket: AppSocket, state: GameEventState) {
  const { currentRoom, playerId, userRole, view, sentences, countdown, gameEndData } = state
  ...
  function handleGameEnded(data: GameEndData) {
    ...
    setTimeout(() => { view.value = 'FINISHED' }, 6200)
  }

  function handleGameStart(_data: { firstSentence: string[]; gameStartTime: number }) {
    view.value = 'GAME'
  ...
  function handleGameForceReset(data: { room: RoomState }) {
    ...
    if (isWipedByJanitor) {
      socket.emit('join_as_player', { roomCode: data.room.roomCode }, (res: any) => {
        if (res.success) {
