...
if (!_JWT_SECRET || _JWT_SECRET.length < 32) {
  console.error('[FATAL] JWT_SECRET is missing or unsafe (min 32 chars required). Exiting.');
  process.exit(1);
}
...
export function generateSessionToken(playerId: string, roomCode: string): string {
  return jwt.sign({ playerId, roomCode }, JWT_SECRET, { expiresIn: '24h', algorithm: 'HS256' });
}
