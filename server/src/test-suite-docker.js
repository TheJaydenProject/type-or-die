...
async function runInfrastructureTests(runner) {
    ...
    `);
		...
		runner.assert(
			count > 0,
			`Sentences table has ${count} active sentences (need at least 1)`,
		);
	});
    ...
    `);
		runner.assert(
			...
			"No valid sentences found in database",
		);
	});
}
...
async function runSocketTests(runner) {
	...
	let socket1, socket2, testRoomCode, _player1Id, _player2Id;
			...
			socket1.emit(
				...
				(response) => {
					try {
						runner.assert(
							response.success,
							`Room creation failed: ${response.error}`,
						);
				...
				socket2.emit(
					...
					(response) => {
						try {
							runner.assert(
								response.success,
								`Join room failed: ${response.error}`,
							);
							runner.assert(
								response.playerId,
								"No player ID returned for player 2",
							);
							runner.assert(
								response.role === "PLAYER",
								"Player should join as PLAYER role",
							);
							_player2Id = response.playerId;
							resolve();
						} catch (err) {
							reject(err);
						}
			...
			socket1.emit(
				...
				(response) => {
					try {
						runner.assert(
							response.success,
							`Settings change failed: ${response.error}`,
						);
						resolve();
					} catch (err) {
						reject(err);
			...
			socket2.emit(
				...
				(response) => {
					try {
						runner.assert(
							!response.success,
							"Non-host should not be able to change settings",
						);
						resolve();
					} catch (err) {
						reject(err);
		...
		return new Promise((resolve, reject) => {
			...
			socket1.on("countdown_start", (data) => {
				...
				runner.assert(
					...
					"Sentences should be an array",
				);
				runner.assert(
					data.sentences.length === 10,
					`Expected 10 sentences, got ${data.sentences.length}`,
				);
			});
			...
			socket1.emit("start_game", { roomCode: testRoomCode }, (response) => {
				try {
					runner.assert(
						response.success,
						`Start game failed: ${response.error}`,
					);

					setTimeout(() => {
						runner.assert(
							countdownReceived,
							"Did not receive countdown_start event",
						);
						resolve();
					}, 500);
				} catch (err) {
		...
		return new Promise((resolve, reject) => {
			...
			setTimeout(() => {
				try {
					runner.assert(
						gameStartReceived,
						"Did not receive game_start event after countdown",
					);
					resolve();
				} catch (err) {
					reject(err);
				...
				socket.emit(
					...
					(response) => {
						try {
							runner.assert(
								response.success,
								"Failed to create room for logic test",
							);
		...
		return new Promise((resolve, reject) => {
			...
			socket.on("player_progress", (data) => {
				if (data.playerId === playerId) {
					...
					runner.assert(
						data.charIndex > 0,
						"Character index should increment after typing",
					);
				}
			});
			...
			setTimeout(() => {
				...
				setTimeout(() => {
					try {
						runner.assert(
							progressReceived,
							"Did not receive player_progress event",
						);
						resolve();
					} catch (err) {
						reject(err);
...
async function runRoomManagerTests(runner) {
	...
	await runner.test(
		"Empty room deletes immediately (Polling check)",
		async () => {
					...
					socket.emit(
						...
						async (response) => {
							try {
								...
								// Verify room exists
								const roomDataBefore = await runner.redis.get(
									`room:${roomCode}`,
								);
								runner.assert(
									roomDataBefore,
									"Room should exist after creation",
								);
								...
								for (let elapsed = 100; elapsed <= 3000; elapsed += 100) {
									...
									const roomStillExists = await runner.redis.get(
										`room:${roomCode}`,
									);
									...
									if (!roomStillExists && !deleted) {
										...
										console.log(
											`    [DEBUG] ✓ Room DELETED at ${deletionTime}ms after socket close`,
										);
										break;
									}
								}
								...
								// Final assertion
								const roomDataAfter = await runner.redis.get(
									`room:${roomCode}`,
								);
								runner.assert(
									!roomDataAfter,
									"Room should be deleted immediately when empty",
								);
	...
	await runner.test("Room creation increments global count", async () => {
		...
		const before = beforeCount ? parseInt(beforeCount, 10) : 0;
					...
					async (response) => {
						try {
							if (response.success) {
								...
								const after = afterCount ? parseInt(afterCount, 10) : 0;
								runner.assert(
									after > before,
									`Room count should increment (before: ${before}, after: ${after})`,
								);
								socket.close();
			...
			socket.on("connect", () => {
				socket.emit(
					...
					async (response) => {
						try {
							if (response.success) {
								const roomData = await runner.redis.get(
								...
								runner.assert(
									room.players[response.playerId],
